  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        <Link href={"/"} className="text-[35px] text-white">
          RoadScan Pro
        </Link>
      </div>
              ...
              {menuItems.map((menuItem) => (
                <li key={menuItem.id}>
                  <label
                    ...
                    className={`group relative cursor-pointer flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-white
                             ${pathName.includes(menuItem.id) && "bg-white"}
                            `}
                  >
                    {menuItem.icon}
                    {menuItem.label}
